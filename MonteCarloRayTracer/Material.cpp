...
DiffuseLambertian::DiffuseLambertian(const dvec3& color, double reflectance_ratio)
	: Material(color), reflectance{ reflectance_ratio / M_PI }
{
...
// The Bidirectional Reflectance Distribution Function of each material determines how incoming rays interact with the material in terms of direction and energy (importance) distribution.

// Perfect reflection, total conservation of energy (importance)
...
// Both perfectly reflected part and refracted part, total conservation of energy (importance)
std::vector<Ray> Glass::BRDF(const std::shared_ptr<Ray>& incoming) const
{
	double reflected_importance{ 0.0 };
	double refracted_importance{ 0.0 };

	vec3 N = incoming->inside_transparent_object ? -incoming->target->getNormal(incoming->end) : incoming->target->getNormal(incoming->end);
	...
	if (incoming->depth < MAX_RAY_DEPTH)
	{
		...
		else
		{
			...
			double R0 = incoming->inside_transparent_object ?
				glm::pow(((this->reflective_index - AIR_REFLECTIVE_INDEX) / (AIR_REFLECTIVE_INDEX + this->reflective_index)), 2.0)
	...
	float reflective_ratio = incoming->inside_transparent_object ? this->reflective_index / AIR_REFLECTIVE_INDEX : AIR_REFLECTIVE_INDEX / this->reflective_index;
