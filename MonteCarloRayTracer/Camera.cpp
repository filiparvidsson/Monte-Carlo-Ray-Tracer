...
Camera::Camera(dvec3 obs1, dvec3 obs2)
	: obs1{obs1}, obs2{obs2}, main_obs{ true }
{
	//Camera plane
	plane[0][0] = dvec3(0, -1, -1); //Left bottom corner
	plane[0][1] = dvec3(0, 1, -1); //Right bottom corner
	plane[1][0] = dvec3(0, -1, 1); //Left top corner
	plane[1][1] = dvec3(0, 1, 1); //Right top corner
...
void Camera::render(Scene& scene) {
	...
	dvec3 p_start = Camera::main_obs ? obs1 : obs2;
	dvec3 p_end;
	...
	for (size_t j = 0; j < RESOLUTION; ++j) {
		for (size_t i = 0; i < RESOLUTION; ++i) {
			...
			p_end = dvec3(0.0, i*PIX_DELTA-(1.0-PIX_DELTA), j*PIX_DELTA-(1.0-PIX_DELTA));
